...
Browser (Next.js + shadcn + TanStack Query)
    → Next :3000  (UI only; rewrites /api → Express)
        → Express :8787
            ...
            → tools (vault fs, project files, dockerode sandbox, tesseract/pdfjs, docx)
...
`preferModel` from the composer changes the generate model only. Store and tools stay with the router. Keyword guards may still correct store (plant data) or force coder + `store: none` on coding turns.
...
Coding turns use a file-writing prompt. A keyword guard forces `store: none` and `model: coder` (HTML/CSS/JS/Python, including typos like “genrate”) so plant SOP retrieve does not starve file writing. Named fences are saved to `data/vault/projects/<conversationId>/` on `done` (filename-only fences merge with the next body; `# app.py` first lines count as names; unnamed `html`/`css`/`js`/`py` fences still write `index.html` / `styles.css` / `main.js` / `main.py`). The loop does not start Docker. Studio Preview/Run bind-mounts that folder. HTML preview is nginx on loopback; the workbench iframe loads `GET /sandbox/preview/:id/index.html` (Next `/api`) so CSS/JS stay same-origin. GET `/projects/:id` includes file contents.
